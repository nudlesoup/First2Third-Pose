...
class MarshDataset(data.Dataset):
    ...
    def __init__(self, path, mode, stacked_opticalflow, directories, keys, load_frame_count, ego_list, train_transform=None, val_transform=None):
        self.dir_path=path
        self.mode = mode
    ...
    def easy_negative_pair(self,index):
        ego_images = 0
        front_images = 0
        opticalflow_ego = 0
        opticalflow_front = 0
        return ego_images, front_images,opticalflow_ego,opticalflow_front
...
def get_loader(train_path,val_path,batch_size,shuffle,stacked_opticalflow, num_workers,train_transform, val_transform):
    ...
    load_frame_count_v = load_frame_count("val")
    ego_train_list = create_ego_list_train(train_path, train_key_names, load_frame_count_t)
    training_set = MarshDataset(path=train_path, mode='train',stacked_opticalflow=stacked_opticalflow,directories=train_directories,keys=train_key_names ,load_frame_count=load_frame_count_t,ego_list=ego_train_list,train_transform=train_transform,val_transform=val_transform)
    ...
    val_directories=sorted(glob.glob(os.path.join(val_path, "*", "")))
    val_key_names = []
    for line in val_directories:
        videoname = line.split('/')[-2]
        val_key_names.append(videoname)
    ego_val_list = create_ego_list_val(val_path, val_key_names, load_frame_count_v)
    validation_set = MarshDataset(path=val_path, mode='val', stacked_opticalflow=stacked_opticalflow,directories=val_directories,keys=val_key_names ,load_frame_count=load_frame_count_v,ego_list=ego_val_list,train_transform=train_transform,val_transform=val_transform)
    val_loader = DataLoader(dataset=validation_set, batch_size=batch_size, shuffle=True, num_workers=num_workers)
