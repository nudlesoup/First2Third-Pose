...
def get_loader(train_path,val_path,batch_size,shuffle,stacked_opticalflow, num_workers,train_transform, val_transform):
    ...
    val_listed_activities = activity_creator(val_key_names)
    ego_val_list = create_ego_list_val(val_path, val_key_names, load_frame_count_v)
    validation_set = MarshDataset(path=val_path, mode='val', stacked_opticalflow=stacked_opticalflow,directories=val_directories,keys=val_key_names ,load_frame_count=load_frame_count_v,ego_list=ego_val_list,listed_activities=val_listed_activities,train_transform=train_transform,val_transform=val_transform)
