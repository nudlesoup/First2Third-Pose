...
from dataset.crloader_hard_train import get_loader

from torch.utils.tensorboard import SummaryWriter

writer = SummaryWriter('summary/Hard_pair_resnet')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (ego_frame, front_frame, ego_optical, front_optical, target) in enumerate(train_loader):
        ...
        if i % 4000 == 0 and i is not 0:
            torch.save(model.state_dict(), os.path.join(args.model_path,
                                                        'fc2-update-two-stream-twores_hard_multi-{}-iter-{}.ckpt'.format(
                                                            epoch + 1, str(i))))
        ...
        if i % 100 == 0 and i is not 0:  # every 1000 mini-batches...
            ...
            writer.add_scalar('Hard_pair_resnet',
                              running_loss / 100,
...
def main():
    ...
    checkpoint = torch.load('/home/adhamanaskar/Research/Siamese_Ameya/checkpoint_easy/004_checkpoint.pth.tar')
    ...
    train_loader = get_loader(train_path=args.train_data_path, batch_size=32, shuffle=True, stacked_opticalflow=10,
                              num_workers=1,
    ...
    for epoch in range(epochs_start, args.epochs):
        # adjust_learning_rate(optimizer, epoch)

        train(train_loader, model, criterion, optimizer, epoch)
        checkpoint_name = "%03d_%s" % (epoch + 1, "hard_checkpoint.pth.tar")
        save_checkpoint({'epoch': epoch + 1, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()},
                        checkpoint_name, args.model_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_data_path', metavar='TrainDIR',
                        help='path to dataset')
