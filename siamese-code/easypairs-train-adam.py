...
from dataset.marsheasyloader_train import get_loader

from torch.utils.tensorboard import SummaryWriter

writer = SummaryWriter('summary/Easy_ADAM_Train_Siamese')
    ...
    for i, (ego_frame, front_frame, ego_optical, front_optical, target) in enumerate(train_loader):
        ...
        if i % 4000 == 0 and i is not 0:
            torch.save(model.state_dict(), os.path.join(args.model_path,
                                                        'twores_easy_multi-{}-iter-{}.ckpt'.format(
                                                            epoch + 1, str(i))))
        ...
        if i % 100 == 0 and i is not 0:  # every 1000 mini-batches...
            ...
            writer.add_scalar('Easy_ADAM_Train_Siamese',
                              running_loss / 100,
...
def main():
    ...
    for epoch in range(epochs_start, args.epochs):
        ...
        checkpoint_name = "%03d_%s" % (epoch + 1, "easy_checkpoint.pth.tar")
